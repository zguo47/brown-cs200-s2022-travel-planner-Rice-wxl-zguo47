...
/**
 * This is the Dijkstra algorithm that calculates the route based on edge
 * weights.
...
public class Dijkstra<V, E> implements IDijkstra<V, E> {
    ...
    /**
     * finds the shortest path between the start and end vertices in the input
     * graph, based on the input edge weight.
    ...
    public List<E> getShortestPath(IGraph<V, E> graph, V source, V destination,
        ...
        // the data structure that stores each vertex's total edge weights from
        // the source.
        ...
        Comparator<V> vertexByRoute = (vertex1, vertex2) -> {
            return Double.compare(vWithWeight.get(vertex1),
                    vWithWeight.get(vertex2));
        };

        // the PriorityQueue which is used to order the vertices by total
        // edge weights.
        ...
        while (!vQueue.isEmpty()) {
            // The checkingVertex is the element of vQueue with the least total
            // edge weights.
            ...
            if (graph.getOutgoingEdges(checkingVertex) != null) {
                for (E theTransport : graph.getOutgoingEdges(checkingVertex)) {
                    ...
                    // updates the total edge weights for the checkingVertex
                    // from the source.
                    if (vWithWeight.get(checkingVertex)
                            + edgeWeight.apply(theTransport) < vWithWeight.
                            get(neighbor)) {
                        vWithWeight.replace(neighbor,
                                vWithWeight.get(checkingVertex)
        ...
        if (cameFrom.containsKey(destination)) {
            ...
            while (!graph.getEdgeSource(thePath.getFirst()).equals(source)) {
                thePath.addFirst(cameFrom.get(graph.getEdgeSource(
                        thePath.getFirst())));
            }
            return thePath;
        } else {
